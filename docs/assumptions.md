...
| A-UNC-07 | **New (EXP-0011).** The diagnoser's hypothesis *support* contains the truth -- i.e. the declared magnitude range, and the fault taxonomy itself, are not misspecified | `OPEN` -- **threat TV-M6, and the most consequential open assumption in the project** | A support error is the only failure measured here that WORSENS with more observation: the likelihood sharpens onto a set that excludes the truth. Measured for magnitude (-0.133 at 18 s); untested for the taxonomy | **EXP-0012** |
| A-UNC-06 | Fault magnitude is known to the diagnoser | **`REFUTED as a concern` (EXP-0011)** — relaxing it costs at most 3.3 points and 0.0001 at the full window. It was expected to be the dominant unmeasured source of ambiguity; it is not | With free magnitude, bias and scale faults on one channel intersect exactly at $V^*$, so the hypothesis classes genuinely overlap | **EXP-0011** |
